...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    N, T, D = x.size()
    H = h0.size(1)
    output = torch.zeros(N, T, H, dtype=x.dtype, device=x.device)
    cache_list = []

    hidden_state = h0
    for t in range(T):
        xt = x[:, t]
        hidden_state, cache_t = rnn_step_forward(xt, hidden_state, Wx, Wh, b)
...
def rnn_backward(dh, cache):
    ...
    dx_all = torch.zeros(N, T, D, dtype=dh.dtype, device=dh.device)
    dWx_total = torch.zeros(D, H, dtype=dh.dtype, device=dh.device)
    dWh_total = torch.zeros(H, H, dtype=dh.dtype, device=dh.device)
    db_total = torch.zeros(H, dtype=dh.dtype, device=dh.device)
    dh_prev = torch.zeros(N, H, dtype=dh.dtype, device=dh.device)

    for t in reversed(range(T)):
        grad_h = dh[:, t] + dh_prev
        dx_t, dh_prev, dWx_t, dWh_t, db_t = rnn_step_backward(grad_h, cache[t])
...
def CaptioningTrain(rnn_model, image_data, caption_data, lr_decay=1, **kwargs):
  ...
  def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b, attn=None, Wattn=None):
    ...
    input_gate = torch.sigmoid(gates[:, :H])
    forget_gate = torch.sigmoid(gates[:, H:2*H])
    output_gate = torch.sigmoid(gates[:, 2*H:3*H])
    block_input = torch.tanh(gates[:, 3*H:])
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    outputs = torch.zeros(N, T, H, dtype=x.dtype, device=x.device)
    hidden_state = h0
    cell_state = c0

    for t in range(T):
        hidden_state, cell_state = lstm_step_forward(x[:, t], hidden_state, cell_state, Wx, Wh, b)
