...
class RPN(nn.Module):
    ...
    def forward(self, images, bboxes, output_mode='loss'):
        ...
        ##############################################################################
        feat_map = self.feat_extractor(images)

        grid_map = GenerateGrid(feat_map)
        anchor_boxes = GenerateAnchor(self.anchor_list, grid_map)
        ...
        conf_pred, offset_pred, _ = self.prop_module(
            feat_map,
            anchor_boxes.view(-1, 4)[pos_idx],
            pos_idx,
            neg_idx
        )

        loss_conf = ConfScoreRegression(conf_pred, gt_conf)
        loss_reg = BboxRegression(offset_pred, gt_offsets)

        total_loss = w_conf * loss_conf + w_reg * loss_reg
...
class TwoStageDetector(nn.Module):
    ...
    def forward(self, images, bboxes):
        ...
        ##############################################################################
        rpn_loss, conf, proposal_boxes, feat_map, gt_labels, pos_idx, _ = \
            self.rpn(images, bboxes, output_mode='all')

        roi_pooled = torchvision.ops.roi_align(
            feat_map, proposal_boxes,
            output_size=(self.roi_output_w, self.roi_output_h)
        )
        roi_pooled = roi_pooled.mean(dim=(2, 3))  # Spatial average pooling

        logits = self.classifier(roi_pooled)
        loss_cls = F.cross_entropy(logits, gt_labels)

        total_loss = rpn_loss + loss_cls
    ...
    def inference(self, images, thresh=0.5, nms_thresh=0.7):
        ...
        proposal_boxes, conf_scores, feat_map = self.rpn.inference(
        ...
        for b in range(batch_size):
            ...
            pooled_roi = torchvision.ops.roi_align(
                feat_map, [boxes],
                output_size=(self.roi_output_w, self.roi_output_h)
            )
            pooled_roi = pooled_roi.mean(dim=(2, 3))

            logits = self.classifier(pooled_roi)
            probs, labels = logits.softmax(dim=-1).max(dim=-1)

            final_proposals.append(boxes)
            final_conf_probs.append(probs)
            final_class.append(labels)
