...
def svm_loss_naive(W, X, y, reg):
  ...
  for idx in range(num_train):
    x_i = X[idx]
    class_scores = torch.matmul(W.T, x_i)
    correct_score = class_scores[y[idx]]

    for cls in range(num_classes):
        if cls == y[idx]:
            continue

        margin = class_scores[cls] - correct_score + 1
        if margin > 0:
            loss += margin
            dW[:, cls] += x_i
            dW[:, y[idx]] -= x_i
...
def svm_loss_vectorized(W, X, y, reg):
  ...
  row_totals = mask.sum(dim=1)
  mask[torch.arange(num_train), y] -= row_totals

  dW = torch.matmul(X.T, mask) / num_train
  dW += 2 * reg * W
...
def softmax_loss_naive(W, X, y, reg):
    ...
    for idx in range(samples):
        logits = torch.matmul(X[idx].unsqueeze(0), W)
        logits -= logits.max()
        exp_vals = torch.exp(logits)
        probs = exp_vals / exp_vals.sum()

        total_loss -= torch.log(probs[0, y[idx]])

        for cls in range(classes):
            prob = probs[0, cls]
            if cls == y[idx]:
                grad[:, cls] += (prob - 1) * X[idx]
            else:
                grad[:, cls] += prob * X[idx]
