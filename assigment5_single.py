...
for img, anc, prop in zip(img_list, coord_trans(anc_list[:, 0:1, 3:4, 3:4, :], w_list, h_list), \
  ...
  def IoU(proposals, bboxes):
    ...
    flat_proposals = proposals.view(B, A * H * W, 4)

    p_x1, p_y1, p_x2, p_y2 = flat_proposals.unbind(dim=-1)
    b_x1, b_y1, b_x2, b_y2, _ = bboxes.unbind(dim=-1)

    p_x1 = p_x1.unsqueeze(2)
    p_y1 = p_y1.unsqueeze(2)
    p_x2 = p_x2.unsqueeze(2)
    p_y2 = p_y2.unsqueeze(2)

    b_x1 = b_x1.unsqueeze(1)
    b_y1 = b_y1.unsqueeze(1)
    b_x2 = b_x2.unsqueeze(1)
    b_y2 = b_y2.unsqueeze(1)

    inter_x1 = torch.maximum(p_x1, b_x1)
    inter_y1 = torch.maximum(p_y1, b_y1)
    inter_x2 = torch.minimum(p_x2, b_x2)
    inter_y2 = torch.minimum(p_y2, b_y2)

    inter_area = (inter_x2 - inter_x1).clamp(min=0) * (inter_y2 - inter_y1).clamp(min=0)

    area_proposals = (p_x2 - p_x1) * (p_y2 - p_y1)
    area_bboxes = (b_x2 - b_x1) * (b_y2 - b_y1)

    union_area = area_proposals.unsqueeze(2) + area_bboxes.unsqueeze(1) - inter_area
    iou_mat = inter_area / union_area
...
def detector_forward(self, images, bboxes):
    ...
    grid_map = GenerateGrid(feat)
    anchor_boxes = GenerateAnchor(self.anchor_shapes, grid_map)

    iou_matrix = IoU(anchor_boxes, bboxes)
    pos_idx, neg_idx, gt_conf, gt_offsets, gt_classes = ReferenceOnActivatedAnchors(
        anchor_boxes, bboxes, iou_matrix, neg_thresh=0.2
    )

    pred_conf, pred_offsets, pred_class = self.pred_network(feat, pos_idx, neg_idx)

    loss_conf = ConfScoreRegression(pred_conf, gt_conf)
    loss_reg = BboxRegression(pred_offsets, gt_offsets)
    loss_cls = ObjectClassification(pred_class, gt_classes, self.num_classes)

    total_loss = w_conf * loss_conf + w_reg * loss_reg + w_cls * loss_cls
...
def detector_inference(self, images, thresh=0.5, nms_thresh=0.7):
    ...
    conf, offsets, class_probs = self.pred_network(feat)
    proposals = GenerateProposal(anchor_boxes, offsets)

    batch_size = conf.size(0)

    for b in range(batch_size):
        mask = conf[b] > thresh
        props = proposals[b][mask]
        scores = conf[b][mask]
        cls_probs = class_probs[b][:, mask]
        ...
        top_scores, top_classes = cls_probs.max(dim=0)
        keep_idx = torchvision.ops.nms(props, scores, nms_thresh)

        final_proposals.append(props[keep_idx])
        final_conf_scores.append(scores[keep_idx].unsqueeze(1))
        final_class.append(top_classes[keep_idx].unsqueeze(1))
