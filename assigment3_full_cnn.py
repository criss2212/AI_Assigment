...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    drelu, grads['W2'], grads['b2'] = Linear.backward(dscores, cache_scores)
    dhid = ReLU.backward(drelu, cache_relu)
    dx, grads['W1'], grads['b1'] = Linear.backward(dhid, cache_hidden)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for idx, h_dim in enumerate(hidden_dims):
        self.params[f'W{idx+1}'] = torch.randn(in_dim, h_dim, dtype=dtype, device=device) * weight_scale
        self.params[f'b{idx+1}'] = torch.zeros(h_dim, dtype=dtype, device=device)
        in_dim = h_dim

    self.params[f'W{self.num_layers}'] = torch.randn(in_dim, num_classes, dtype=dtype, device=device) * weight_scale
  ...
  def loss(self, X, y=None):
    ...
    for layer in range(1, self.num_layers):
        current, caches[f'layer{layer}'] = Linear.forward(current, self.params[f'W{layer}'], self.params[f'b{layer}'])
        current, caches[f'relu{layer}'] = ReLU.forward(current)

        if self.use_dropout:
            current, dropout_caches[f'dropout{layer}'] = torch.nn.functional.dropout(
                current,
                p=self.dropout_param['p'],
                training=(mode == 'train')
            )

    scores, caches[f'layer{self.num_layers}'] = Linear.forward(
        current,
    ...
    for layer in range(1, self.num_layers + 1):
        loss += 0.5 * self.reg * torch.sum(self.params[f'W{layer}'] ** 2)

    probs[torch.arange(N), y] -= 1
    dscores = probs / N

    dout, grads[f'W{self.num_layers}'], grads[f'b{self.num_layers}'] = Linear.backward(
        dscores, caches[f'layer{self.num_layers}']
    )
    ...
    for layer in reversed(range(1, self.num_layers)):
        ...
        dout = ReLU.backward(dout, caches[f'relu{layer}'])
        dout, grads[f'W{layer}'], grads[f'b{layer}'] = Linear.backward(dout, caches[f'layer{layer}'])
