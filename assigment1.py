...
def compute_distances_two_loops(x_train, x_test):
  ...
  x_test_squared = torch.sum(x_test_flat ** 2, dim=1)
  cross_term = torch.mm(x_train_flat, x_test_flat.t())
  dists = x_train_squared - 2 * cross_term + x_test_squared
  return dists
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
...
def compute_distances_one_loop(x_train, x_test):
  ...
  dists = torch.cdist(x_train_flat, x_test_flat, p=2) ** 2
  return dists
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
    k_list = k_choices if k_choices is not None else [1, 3, 5, 8, 10, 12, 15, 20, 50, 100]

    fold_size = x_train.shape[0] // num_folds
    x_train_folds = [x_train[i * fold_size: (i + 1) * fold_size] for i in range(num_folds)]
    y_train_folds = [y_train[i * fold_size: (i + 1) * fold_size] for i in range(num_folds)]
...
k_to_accuracies = {}
  ...
  # Replace "pass" statement with your code
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
    k_vals = k_choices or [1, 3, 5, 8, 10, 12, 15, 20, 50, 100]

    x_folds = torch.chunk(x_train, num_folds, dim=0)
    y_folds = torch.chunk(y_train, num_folds, dim=0)

    results = {k: [] for k in k_vals}

    for i in range(num_folds):
        val_x, val_y = x_folds[i], y_folds[i]

        train_x = torch.cat([fold for j, fold in enumerate(x_folds) if j != i], dim=0)
        train_y = torch.cat([fold for j, fold in enumerate(y_folds) if j != i], dim=0)

        distances = compute_distances_no_loops(train_x, val_x)

        for k in k_vals:
            predictions = predict_labels(distances, train_y, k=k)
            acc = (predictions == val_y).float().mean().item()
