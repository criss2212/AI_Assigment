...
def make_adversarial_attack(X, target_y, model):
    ...
    for step in range(100):
        output = model(X_adv)
        desired_score = output[:, target_y]

        if output.argmax(dim=1).item() == target_y:
            print(f"Success at iteration {step}")
            break

        model.zero_grad()
        desired_score.backward()

        grad = X_adv.grad
        perturb = learning_rate * grad / grad.norm()
        X_adv = (X_adv + perturb).detach().requires_grad_()
