...
for lr in lrs:
    for h in h_sizes:
        for reg in regs:
            model_params = {
                'W1': torch.randn(data_dict['X_train'].shape[1], h) * 0.001,
                'b1': torch.zeros(h),
                'W2': torch.randn(h, 10) * 0.001,
                'b2': torch.zeros(10)
            }

            training_stats = nn_train(
                model_params, nn_loss_part2, nn_predict,
                data_dict['X_train'], data_dict['y_train'],
                data_dict['X_val'], data_dict['y_val'],
                learning_rate=lr, reg=reg,
                num_iters=iterations, batch_size=200,
                verbose=False
            )

            final_val_acc = training_stats['val_acc_history'][-1]
            if final_val_acc > best_val:
                best_val = final_val_acc
                best_net = model_params
