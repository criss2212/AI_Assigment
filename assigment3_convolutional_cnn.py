        ...
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    out[n, c, i, j] = torch.max(x[n, c, h0:h1, w0:w1])
...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    pooled_dim = (H // 2) * (W // 2) * F

    self.params['W2'] = torch.randn(pooled_dim, hidden_dim, dtype=dtype, device=device) * weight_scale
...
def __init__(self, input_dims=(3, 32, 32),
    ...
    for idx, out_channels in enumerate(num_filters):
        self.params[f'W{idx+1}'] = torch.randn(out_channels, in_channels, 3, 3, dtype=dtype, device=device) * weight_scale
        self.params[f'b{idx+1}'] = torch.zeros(out_channels, dtype=dtype, device=device)

        if batchnorm:
            self.params[f'gamma{idx+1}'] = torch.ones(out_channels, dtype=dtype, device=device)
            self.params[f'beta{idx+1}'] = torch.zeros(out_channels, dtype=dtype, device=device)

        in_channels = out_channels

    final_height = H // (2 ** len(max_pools))
    final_width = W // (2 ** len(max_pools))
    final_dim = in_channels * final_height * final_width

    self.params[f'W{self.num_layers}'] = torch.randn(final_dim, num_classes, dtype=dtype, device=device) * weight_scale
...
def loss(self, X, y=None):
    ...
    for layer in range(1, self.num_layers + 1):
        ...
        loss += self.reg * torch.sum(weight ** 2)

    dx, dw, db = Linear.backward(dscores, caches['linear'])
    ...
    for layer in reversed(range(1, self.num_layers)):
        if (layer - 1) in self.max_pools:
            dx = MaxPool.backward(dx, pool_caches[f'pool{layer}'])

        dx = ReLU.backward(dx, caches[f'relu{layer}'])

        if self.batchnorm:
            dx, dgamma, dbeta = BatchNorm.backward(dx, caches[f'bn{layer}'])
            grads[f'gamma{layer}'] = dgamma
            grads[f'beta{layer}'] = dbeta

        dx, dw, db = Conv.backward(dx, caches[f'conv{layer}'])
...
def batchnorm_backward(dout, cache):
    ...
    dx_mu1 = dx_norm * inv_std

    dvar = torch.sum(dx_norm * (x - mean) * -0.5 * inv_std**3, dim=0)
    dmean = torch.sum(-dx_mu1, dim=0) + dvar * torch.mean(-2.0 * (x - mean), dim=0)

    dx = dx_mu1 + (dvar * 2 * (x - mean) / N) + (dmean / N)
